% s should be the name of a IEEE cdf file

fid = fopen('ieee300cdf.txt');
size = textscan(fid,"%s%s%s%d%s%s%s%d",1);
busSize = size{1,8};
base = size{1,4};
fileType = size{1,1};
...
if(fileType == "TAPE")
    skip=1;
    caseSize = textscan(fid,"%s%s%s%d%s%s%s%d",1,'headerlines',2+skip);
    busSize = size{1,4};
    base = textscan(fid,"%s%s%s%d%s%s%s%d",1,'headerlines',2+skip);
    base = base{1,4};
else
    skip=0;
end
...
% 19 bus number
textBusData = textscan(fid,'%d%11c%s%d%d%d%f%f%f%f%f%f%f%f%f%f%f%f%d',busSize,'headerlines',2+skip);
...
fid = fopen(s);
textBranchData = textscan(fid,'%d%d%d%d%d%d%f%f%f%d%d%d%d%d%f%f%f%f%f%f%f',branchSize,'headerlines',lineSkipToBranch+skip);
